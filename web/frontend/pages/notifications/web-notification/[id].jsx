...
export default function WebNotification() {
  ...
  const fetchData = async () => {
    try {
      const sessionToken = await getSessionToken(appBridge);
      const response = await axios.get(`${apiUrl}web-notification-detail/${ParamsId}`, {
        headers: {
          Authorization: `Bearer ${sessionToken}`,
        },
      });
      ...
      fetchEmailList(web_notification_data?.notification_id);
    } catch (error) {
      console.error("Error fetching data:", error);
    } finally {
      setLoading(false);
  ...
  return (
    <>
      <div style={{ minHeight: "80vh" }}>
        <FullscreenBar onAction={() => navigate(`/notifications/${webData?.notification_id}`)}>
          <Box paddingBlockStart={"300"} paddingBlockEnd={"300"} paddingInlineStart={"400"} paddingInlineEnd={"400"} width="100%">
            <InlineGrid columns="1fr 1fr auto">
              {loading ? (
                ...
                <>
                  <InlineStack blockAlign="center" wrap={false} gap={"400"}>
                    <Text variant="headingMd">{webData?.title}</Text>
                  </InlineStack>
                  <InlineStack blockAlign="center" wrap={false} gap={"400"}>
                    <Popover
                    ...
                    >
                      <ActionList
                        ...
                        items={emailsList?.map((item) => ({
                          ...
                          onAction: () => {
                            setLoading(true);
                            navigate(`/notifications/web-notification/${item?.id}`);
              ...
              <InlineStack blockAlign="center" gap={"200"}>
                <ButtonGroup variant="segmented">
                  <Button size="medium" textAlign="center" pressed={view === "Windows"} onClick={() => handleView("Windows")}>
                    Windows
                  </Button>
                  <Button size="medium" textAlign="center" pressed={view === "MacOS"} onClick={() => handleView("MacOS")}>
                    MacOS
                  </Button>
                  <Button size="medium" textAlign="center" pressed={view === "Mobile"} onClick={() => handleView("Mobile")}>
                    Mobile
                  </Button>
                </ButtonGroup>
                ...
                >
                  <div className="h-full py-4">
                    <div className="h-full shadow-border scroll_custom">
                      {view === "Windows" ? (
                        <div className="preview_flyout_widget window_preveiew_main w-100 h-full flex justify-center items-center">
                          <div className="window_preveiew relative" style={{ paddingBottom: "100px", paddingRight: "120px" }}>
                        ...
                        <div className="preview_flyout_widget w-100 p-0 h-full">
                          <div className="macos_custom_main h-full flex justify-center items-center">
                        ...
                        <div className="preview_flyout_widget w-100 p-5 h-full">
                              ...
                              <div class="mobile_custom_preview_in">
                                <div className="flex items-center gap-1">
                                  ...
                                  {loading ? (
                                    ...
                                    <p class="text-xss mobile_pre_url m-0">{data.url}</p>
                                  )}
                                </div>
