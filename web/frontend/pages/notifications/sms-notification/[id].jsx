...
export default function SMS() {
  ...
  const fetchData = async () => {
    try {
      const sessionToken = await getSessionToken(appBridge);
      const response = await axios.get(`${apiUrl}sms-notification-detail/${ParamsId}`, {
        headers: {
          Authorization: `Bearer ${sessionToken}`,
        },
      });
      ...
      fetchEmailList(sms_notification_data?.notification_id);
    } catch (error) {
      console.error("Error fetching data:", error);
    } finally {
      setLoading(false);
  ...
  return (
    <>
      <div style={{ minHeight: "80vh" }}>
        <FullscreenBar onAction={() => navigate(`/notifications/${smsData?.notification_id}`)}>
          <Box paddingBlockStart={"300"} paddingBlockEnd={"300"} paddingInlineStart={"400"} paddingInlineEnd={"400"} width="100%">
            <InlineGrid columns="1fr 1fr auto">
              {loading ? (
                ...
                <>
                  <InlineStack blockAlign="center" wrap={false} gap={"400"}>
                    <Text variant="headingMd">{smsData?.title}</Text>
                  </InlineStack>
                  <InlineStack blockAlign="center" wrap={false} gap={"400"}>
                    <Popover
                    ...
                    >
                      <ActionList
                        ...
                        items={emailsList?.map((item) => ({
                          ...
                          onAction: () => {
                            setLoading(true);
                            navigate(`/notifications/sms-notification/${item?.id}`);
