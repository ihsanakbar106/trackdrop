...
export default function Modern() {
  ...
  const { apiUrl, shop, modernTrackingPageStyle, modernTrackingPageTitle, modernTrackingPageHandle,trackingPageType } = useContext(AppContext);
  ...
  const [data, setData] = useState({
    search: "",
    search_tracking_number: 1,
    search_order_with_email: 1,
    store_name: "",
    container_width: "1100",
    container_width_type: "px",
    process_bar_color: "#000000",
    store_information: "",
    phone_icon: "",
  ...
  const [selectedProductsType, setSelectedProductsType] = useState(["none"]);
  ...
  const handleChangeCheckbox = (fieldName, value) => {
    const isTrackingChecked = convertNumberToBoolean(data?.search_tracking_number);
    const isOrderChecked = convertNumberToBoolean(data?.search_order_with_email);

    // If only one is checked and the user tries to uncheck it, do nothing
    if ((isTrackingChecked && isOrderChecked) || (!value)) {
      setData((prevFieldObject) => ({
        ...prevFieldObject,
        [fieldName]: value ? 0 : 1, // Toggle the checkbox value
      }));
    }
  ...
  const handleAddPage = async (btnLoading) => {
    if (trackingPageType === "Modern" && !String(data?.store_name || "").trim()) {
      setErrors((prevErrors) => ({
        ...prevErrors,
    ...
    try {
      let sessionToken = await getSessionToken(appBridge);
      const payload = {
        theme_type: trackingPageType,
        page_name: modernTrackingPageTitle,
        logo: file,
        icon: secondIcon,
        data: {
          handle:modernTrackingPageHandle,
          pageData: data,
            ...
            <Card>
              <BlockStack gap={"400"}>
                ...
                <Checkbox
                    ...
                    disabled={!isOrderChecked} // Disable if the other checkbox is unchecked
                    onChange={() => handleChangeCheckbox("search_tracking_number", data?.search_tracking_number)}

                />
                <Checkbox
                    label="Order number and email"
                    checked={isOrderChecked}
                    disabled={!isTrackingChecked} // Disable if the other checkbox is unchecked
                    onChange={() => handleChangeCheckbox("search_order_with_email", data?.search_order_with_email)}

                />
                </BlockStack>
                {/*<Checkbox
                    label="Tracking number"
                    checked={convertNumberToBoolean(data?.search_tracking_number)}
                    onChange={() => handleChangeCheckbox("search_tracking_number", data?.search_tracking_number)}
                />
                <Checkbox
                    label="Order number and email"
                    checked={convertNumberToBoolean(data?.search_order_with_email)}
                    onChange={() => handleChangeCheckbox("search_order_with_email", data?.search_order_with_email)}
                />*/}
              ...
              <Box padding={"400"}>
                <InlineGrid columns={2} gap={"400"}>
                  {trackingPageType=="Modern"?
                      ...
                      :
                      <TextField
                          label="Theme container width"
                          value={data?.container_width}
                          onChange={(value) => handleChangeValue("container_width", value)}
                          placeholder="Enter your container width"
                          type="number"
                          autoComplete="off"
                          connectedRight={
                            <Select
                                ...
                                onChange={(value) => handleChangeValue("container_width_type", value)}
                                value={data?.container_width_type}
                            />
                          }
                      />
                  }
                  ...
                  <HexColorPicker label="Process bar color"
                                  helpText=""
                                  color={data?.process_bar_color}
                                  onChange={(value) => handleChangeValue("process_bar_color", value)}
                  />
                  {/*{file ? (
              ...
              <Box padding={"400"}>
                <BlockStack gap={"400"}>
                  <ChoiceList
                    ...
                    selected={data?.second_link_type}
                    ...
                    (secondIcon ? (
                      <InlineStack align="space-between" blockAlign="center">
                        <Thumbnail
                          size="large"
                          alt={secondIcon.name}
                          source={validImageTypes.includes(secondIcon.type) ? window.URL.createObjectURL(secondIcon) : NoteIcon}
                        />
            ...
            <Card>
              <BlockStack gap={"400"}>
                <BlockStack>
                  ...
                  <Text as="span" variant="bodyMd" tone="subdued">
                    Incorporate an FAQ section on your tracking page to assist with answering the most common questions customers have regarding their
                    shipping and orders.
                  </Text>
                </BlockStack>
                <Checkbox
      ...
      // },
      {
        ...
        label: (
          <>
            <BlockStack>
              <InlineStack blockAlign="center" gap={"050"}>
                <Text variant="headingSm">No products</Text>
              </InlineStack>
              <BlockStack>
                <InlineStack blockAlign="center" gap={"050"}>
                  <Text tone="subdued">no product recommendations.</Text>
                </InlineStack>
              </BlockStack>
            </BlockStack>
          </>
